...
def RaisedCosinePulse(t, Freq, Amplitude):
	...
	return P
"""
def RaisedCosinePulse(t, time, Freq, Amplitude):
	"""
	Raised-Cosine Pulse
	...
	# TMin = 1e-6 / 60.0
	# Number = time / TMin
	# t_source = np.linspace(0.0, time, Number)
	# P  = np.zeros((N,),dtype=np.float32)
	for m in range(0,N):
		if t[m] < time:
			P[m] = Amplitude * cos(2*pi*Freq*t[m])
		else: break

	print(m)
	print(t.shape)
	print(t.ravel()[np.flatnonzero(t)].shape)
	print(time)
	print(P.shape)
	print(P.ravel()[np.flatnonzero(P)].shape)
	return P
...
class Signals():
	...
	def __init__(self, Name="RaisedCosine",time = 1,  Amplitude=1, Frequency=1, N_Cycles=1):
	...
	def generate(self, t):
		if self.Name == "RaisedCosine":
			return RaisedCosinePulse(t, self.time, self.Frequency, self.Amplitude)
